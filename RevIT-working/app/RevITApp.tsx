...
export default function RevITApp({ initialUser = null, cloudEnabled = false }: { initialUser?: InitialUser | null; cloudEnabled?: boolean }) {
  ...
  const progressMetrics = useMemo(() => ({
    questionsAnswered: cloudEnabled
      ? activity.reduce((sum, day) => sum + day.questions_answered, 0)
      : attempts.length,
    aiMessages: activity.reduce((sum, day) => sum + day.review_count, 0),
    streakDays: streak.longest,
    examsCreated: exams.length,
  }), [activity, attempts.length, cloudEnabled, exams.length, streak.longest]);
...
useEffect(() => {
  ...
  // The metrics are the local sources of truth; progression changes are applied inside this effect.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function recordProgressOnlyEvent(eventKey: string, xp: number, metrics = progressMetrics) {
    await persistProgressEvents([{
      eventKey,
      xp,
      activityDate: dateKeyInTimeZone(new Date(), preferences.timezone),
    }], metrics);
  }
      ...
      <section className="workspace">
        ...
        {levelUp !== null && (
          <div className="level-up-toast" role="status" aria-live="polite">
            ...
            <span>Level up</span>
            <strong>You reached Level {levelUp}</strong>
            <p>Keep improving.</p>
          </div>
        )}
