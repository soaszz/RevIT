...
function localProgressKey(ownerKey: string) {
  return `revit-progression-${LOCAL_PROGRESS_VERSION}:${ownerKey}`;
}

function emptyLocalStore(): LocalProgressStore {
  return { totalXp: 0, eventKeys: [], unlocked: [] };
}
...
function unlockLocalAchievements(store: LocalProgressStore, metrics: ProgressMetrics) {
  ...
  store.unlocked.push(...newlyUnlocked.map((achievement) => ({
...
export function recordLocalProgressEvents(
  ownerKey: string,
...
export async function recordCloudProgressEvents(
...
function cloudQueueKey(userId: string) {
  return `revit-progression-queue-${CLOUD_QUEUE_VERSION}:${userId}`;
}
