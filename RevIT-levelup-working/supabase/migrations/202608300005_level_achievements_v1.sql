...
on conflict (id) do update set name = excluded.name, description = excluded.description, icon = excluded.icon, xp_reward = excluded.xp_reward, condition_type = excluded.condition_type, condition_value = excluded.condition_value;
...
create trigger on_auth_user_progress_created after insert on auth.users for each row execute function public.handle_new_user_progress();
...
create policy "user_progress_insert_own" on public.user_progress for insert to authenticated with check ((select auth.uid()) = user_id);
drop policy if exists "user_progress_update_own" on public.user_progress;
create policy "user_progress_update_own" on public.user_progress for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
...
-- Reuse activity_events as the idempotency ledger for both study data and XP.
...
begin
  ...
  into questions_total, ai_total from public.daily_activity where user_id = owner;
  ...
  with eligible as (
    select achievement.id from public.achievements achievement
  ...
  from inserted join public.achievements achievement on achievement.id = inserted.achievement_id;

  if reward_total > 0 then
    update public.user_progress set total_xp = total_xp + reward_total, updated_at = now() where user_id = owner;
  end if;

  return query select achievement.* from public.achievements achievement where achievement.id = any(unlocked_ids) order by achievement.condition_value, achievement.created_at;
end;
$$;
